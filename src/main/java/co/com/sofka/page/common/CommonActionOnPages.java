...
public class CommonActionOnPages extends BaseSikulix{
    ...
    public CommonActionOnPages(WebDriver driver, Duration seconds, boolean explicitTime) {
        try{
            if(driver == null)
                LOGGER.warn(WEBDRIVER_NULL_MESSAGE);
            ...
            if(explicitTime)
                ...
                webDriverImplicitWait(driver, seconds.getSeconds());
    ...
    private void setWebDriverExplicitWait(WebDriver driver, Duration seconds){
        try{
    ...
    //Implicit wait.
    private void webDriverImplicitWait(WebDriver driver, long seconds){
        try{
